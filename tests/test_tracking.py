...
import pytest
from app.services.tracking import (WebSocketManager, handle_acknowledgment,
...
async def test_retrieve_nearby_drivers_available():
    with patch(
        "app.services.tracking.find_nearby_drivers", return_value=[{"driver_id": "1"}]
    ):
...
async def test_driver_location_update_kafka_failure():
    driver_data = {"driver_id": "1", "latitude": 37.7749, "longitude": -122.4194}
    with patch(
        "app.services.tracking.publish_to_kafka", side_effect=Exception("Kafka error")
    ):
