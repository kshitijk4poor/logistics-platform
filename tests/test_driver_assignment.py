...
from app.services.assignment import find_nearest_driver, assign_driver_to_booking

@pytest.mark.asyncio
async def test_find_nearest_driver_success():
    location = {"latitude": 37.7749, "longitude": -122.4194}
    with patch('app.services.assignment.get_available_drivers', return_value=[{"driver_id": "1", "distance": 5}]):
...
async def test_find_nearest_driver_with_radius_expansion():
    location = {"latitude": 37.7749, "longitude": -122.4194}
    with patch('app.services.assignment.get_available_drivers', side_effect=[[], [{"driver_id": "2", "distance": 10}]]):
        driver = await find_nearest_driver(location)
...
async def test_assign_driver_to_booking_success():
    ...
    with patch('app.services.assignment.is_driver_available', return_value=True), \
         patch('app.services.assignment.update_booking_with_driver', return_value=True):
...
async def test_assign_driver_to_non_existent_booking():
    ...
    driver_id = "driver_123"
    with patch('app.services.assignment.is_driver_available', return_value=True), \
         patch('app.services.assignment.update_booking_with_driver', side_effect=Exception("Booking not found")):
