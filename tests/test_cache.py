...
async def test_cache_driver_availability(caching_service):
    ...
    with patch.object(caching_service, "set", return_value=True) as mock_set:
        result = await caching_service.cache_driver_availability(
            driver_id, availability_data
        )
        assert result is True
        mock_set.assert_called_once_with(
            f"driver_availability:{driver_id}", availability_data
        )
...
async def test_retrieve_cached_driver_availability(caching_service):
    ...
    availability_data = {"available": True}
    with patch.object(
        caching_service, "get", return_value=availability_data
    ) as mock_get:
...
async def test_redis_connection_failure(caching_service):
    driver_id = "driver_123"
    with patch.object(
        caching_service, "get", side_effect=Exception("Redis connection error")
    ):
