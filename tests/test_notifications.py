...
async def test_notify_driver_assignment_success(notification_service):
    ...
    with patch.object(
        ...
        result = await notification_service.notify_driver_assignment(
            driver_id, booking_id
        )
        assert result is True
        mock_send.assert_called_once_with(
            driver_id, f"New booking assigned: {booking_id}"
        )
...
async def test_notify_nearby_drivers_success(notification_service):
    ...
    with patch.object(
        ...
        results = await notification_service.notify_nearby_drivers(
            driver_ids, booking_id
        )
        assert all(results)
...
async def test_notify_nearby_drivers_partial_failure(notification_service):
    ...
    with patch.object(
        notification_service,
        "send_notification",
        side_effect=[True, Exception("Notification failure"), True],
    ):
        results = await notification_service.notify_nearby_drivers(
            driver_ids, booking_id
        )
...
async def test_notify_connected_and_disconnected_drivers(notification_service):
    ...
    booking_id = "booking_123"
    with patch.object(
        notification_service, "send_notification", side_effect=[True, False]
    ):
        results = await notification_service.notify_nearby_drivers(
            driver_ids, booking_id
        )
