...
from app.services.messaging import KafkaProducerService, KafkaConsumerService
...
async def test_kafka_connection_handling():
    producer = KafkaProducerService()
    consumer = KafkaConsumerService()
    with patch.object(producer, 'start', return_value=True) as mock_start_producer, \
         patch.object(consumer, 'start', return_value=True) as mock_start_consumer, \
         patch.object(producer, 'stop', return_value=True) as mock_stop_producer, \
         patch.object(consumer, 'stop', return_value=True) as mock_stop_consumer:
