...
from app.services.admin.admin_service import get_fleet, add_vehicle, update_vehicle, delete_vehicle
...
async def add_vehicle_route(vehicle_data: VehicleSchema, db: AsyncSession = Depends(get_db)):
...
async def update_vehicle_route(vehicle_id: int, vehicle_data: VehicleUpdate, db: AsyncSession = Depends(get_db)):
